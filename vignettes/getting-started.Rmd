...
The core of `transitmattr` is the **query builder**: you create a query object
for your transit mode, chain methods to set context (line, stop, date), and then
call a terminal method to fetch the data.
...
All query methods return a **data frame** ready for analysis — no conversion needed.
...
```{r query-step-by-step, eval = FALSE}
q <- tm_rt_query("Red")
q$stop("Harvard")
q$date("2024-01-15")
...
Every query object has a `$stations()` method that returns a data frame of all
stops for that line or route. Use this to find the exact station name to pass to
`$stop()`, `$from_stop()`, or `$to_stop()`.
...
You can also call `$directions()` to see which direction names apply to the line:
...
These terminal methods need a date (a `"YYYY-MM-DD"` string or a `Date` object).
You can pass the date directly to the terminal method or set it earlier with
`$date()`.
...
For multi-day analysis use `$date_range(start, end)` instead of `$date()`.
...
Aggregate endpoints return long-run averages over a date range, useful for
trend analysis.
...
`$headways()`, `$dwells()`, `$aggregate_headways()`, and `$aggregate_dwells()`.
Commuter rail additionally supports `$travel_times()` and
`$aggregate_travel_times()`.

```{r other-modes, eval = FALSE}
...
> **Tip:** Terminus stations on commuter rail and ferry may have incomplete stop
> IDs in one direction. `$stop()` will warn you if you pick a terminus.

## Step 9: Make a quick plot

Query methods return data frames, so you can pipe them directly into `ggplot2`:
...
|---|---|
| `tm_healthcheck()` | API status |
| `tm_git_id()` | Server git commit SHA |
| `tm_routes()` | All routes grouped by mode |
| `tm_bus_routes()` | Bus route IDs (local) |
| `tm_cr_routes()` | Commuter rail route IDs (local) |
| `tm_ferry_routes()` | Ferry route IDs (local) |
...
| `tm_facilities()` | All MBTA stations |
| `tm_service_ridership_dashboard()` | System-wide ridership summary |
...
| `tm_bus_query(route)` | Route ID from `tm_bus_routes()` |
| `tm_cr_query(route)` | Route ID from `tm_cr_routes()` |
| `tm_ferry_query(route)` | Route ID from `tm_ferry_routes()` |
