...
Build a Red Line query, set a date range, and call `$ridership()`. The
result is already a `data.frame` — no
...
> **What to notice:** Weekends typically show lower ridership. If a
> weekday is unusually low, there may have been a service disruption —
> check
...
This uses the **aggregate** endpoint, which gives you long-run daily
averages rather than individual trips.
...
than normal, usually because of track issues. Unlike most endpoints,
this one returns a **list** (not a data.frame) because its structure is
more complex.
